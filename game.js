...
function createMoon() {
    ...
    moonElement.classList.add('luna'); // Añade la clase de luna
    document.getElementById('game-container').appendChild(moonElement); // Añade la luna al contenedor del juego
}
...
function showCredits() {
    // Crear un elemento para la pantalla de créditos
    ...
    creditsScreen.style.fontSize = '24px';

    // Crear las líneas de créditos
    const creditsLines = [
        '¡Felicidades!',
        'Has completado el juego.',
        'Desarrollado por: Jesús Andrés',
        'Colaboraciones: SunoIA, Dall-e, Blackbox',
        'Gracias por jugar.'
    ];

    // Agregar cada línea con efecto de cascada
    creditsLines.forEach((line, index) => {
        const lineElement = document.createElement('p');
        lineElement.textContent = line;
        lineElement.classList.add('cascade');
        lineElement.style.animationDelay = `${index * 0.5}s`; // Retraso para el efecto de cascada
        creditsScreen.appendChild(lineElement);
    });

    // Agregar el botón para reiniciar el juego
    const restartButton = document.createElement('button');
    restartButton.id = 'restart-button';
    restartButton.textContent = 'Reiniciar Juego';
    creditsScreen.appendChild(restartButton);

    // Agregar evento al botón para reiniciar el juego
    restartButton.addEventListener('click', () => {
        document.body.removeChild(creditsScreen); // Eliminar la pantalla de créditos
...
function levelUp() {
    if (level < 2) { // Verifica si el nivel es menor que 50
        ...
        showCredits(); // Mostrar créditos al alcanzar el nivel máximo
    }
}
    ...
    let bossInterval = setInterval(() => {
        ...
        if (path.length > 0) {
            ...
            if (bossPosition === pacmanPosition) {
                ...
                // Continuar el movimiento del boss
                const newPath = aStar(bossPosition, pacmanPosition, obstacles);
                if (newPath.length > 0) {
                    // Mover el boss al siguiente paso en el camino
                    const nextNextPosition = newPath[1]; // El siguiente paso en el camino
                    cells[bossPosition].classList.remove('boss'); // Eliminar el boss de la posición anterior
                    bossPosition = nextNextPosition; // Actualizar la posición del boss
                    cells[bossPosition].classList.add('boss'); // Añadir el boss a la nueva posición
                }
            }
        }
...
document.addEventListener('DOMContentLoaded', () => {
    createGrid();
    createMoon(); // Crear la luna en la parte superior derecha
    displayPacman();
    placePoints();
    placeEnemies();
    placeObstacles();
